...
/* On creer un nouvel automate, dont on ne recopiera que les états
 * accessibles et les transitions qui n'ont pas de lien avec des états
 * non accessibles.
 * On a pris le choix de ne pas regarder si l'alphabet est réduit
 * après la construction d'un tel automate.
 */
Automate * automate_accessible( const Automate * automate){
  ...
  // états accessibles depuis chaque états initial.
  for (
  ...
  // (même alphabet, même états initiaux)
  // les etats correspondent
...
Automate * creer_automate_des_prefixes( const Automate* automate ){
  ...
  ajouter_elements(res->finaux, get_etats( automate ));
  return res;
}

Automate * creer_automate_des_suffixes( const Automate* automate ){
  Automate * res = copier_automate(automate);
  ajouter_elements(res->initiaux, get_etats( automate ));
  return res;
}
...
/* Renvoie l'automation destination, dont le nom de tous les états est
  ...
  for( it2 = premier_iterateur_table( dst->transitions );
    ...
    for( it1 = premier_iterateur_ensemble( fins );
	 ...
	 it1 = iterateur_suivant_ensemble( it1 )
      ...
      ajouter_transition( res, get_valeur(t_origine), cle->lettre, get_valeur(t_fin));
    }
  }
  return res;
}
...
Automate * creer_automate_de_concatenation( const Automate * automate1,
  ...
  for(
      it1 = premier_iterateur_ensemble( get_finaux( automate1 ));
      ! iterateur_ensemble_est_vide( it1 );
      it1 = iterateur_suivant_ensemble( it1 )
      ){
    for(
	it2 = premier_iterateur_ensemble( get_initiaux( new_automate2 ));
	! iterateur_ensemble_est_vide( it2 );
	it2 = iterateur_suivant_ensemble( it2 )
	){
