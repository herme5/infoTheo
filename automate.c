...
Ensemble* etats_accessibles( const Automate * automate, int etat ){
	...
	for (
		...
		const Ensemble * voisins = delta( automate, res, get_element( it ));
...
Automate * creer_automate_de_concatenation( const Automate* automate1, const Automate* automate2 ){
	...
	ajouter_elements( res->alphabet, 
			  creer_union_ensemble( get_alphabet( automate1 ),
						get_alphabet( automate2 )));
	ajouter_elements( res->etats, 
			  creer_union_ensemble( get_etats( automate1 ),
						get_etats( automate2 )));
	ajouter_elements( res->initiaux, 
			  creer_union_ensemble( get_initiaux( automate1 ),
						get_initiaux( automate2 )));
	ajouter_elements( res->finaux, 
			  creer_union_ensemble( get_finaux( automate1 ),
						get_finaux( automate2 )));
	for(
		...
		for(
		    it2 = premier_iterateur_ensemble( get_initiaux( automate2 ));
		    ! iterateur_ensemble_est_vide( it2 );
		    it2 = iterateur_suivant_ensemble( it2 )
		    ){
